...
PROFILE_COORDINATES = {
    0: {"text_box_x": 75, "text_box_y": 392, "send_x": 447, "send_y": 392},
    1: {
        "text_box_x": 529,  # Profile 1 coordinates (500px offset)
        "text_box_y": 392,
        "send_x": 908,
        "send_y": 392,
    },
    2: {
        "text_box_x": 987,  # Profile 2 coordinates (1000px offset)
        "text_box_y": 392,
        "send_x": 1372,
        "send_y": 392,
    },
    3: {
        "text_box_x": 70,  # Profile 3 coordinates (1500px offset)
        "text_box_y": 773,
        "send_x": 440,
        "send_y": 773,
    },
    4: {
        "text_box_x": 528,  # Profile 3 coordinates (1500px offset)
        "text_box_y": 773,
        "send_x": 907,
        "send_y": 773,
    },
    5: {
        "text_box_x": 990,  # Profile 3 coordinates (1500px offset)
        "text_box_y": 773,
        "send_x": 1360,
        "send_y": 773,
    },
...
def human_type(text):
    """
    Uses '-' as separator:
    left side = human typing
    right side = clipboard paste (emoji / unicode)
    """
    ...
    for i, part in enumerate(parts):
        ...
        # If it contains emoji section (we assume right side of "-")
        if i > 0:
            # Paste via clipboard (emoji-safe)
            pyperclip.copy(part)
            pyautogui.hotkey("ctrl", "v")  # Windows/Linux
            # pyautogui.hotkey("command", "v")  # macOS
        else:
            # Human-like typing for normal text
            words = part.split(" ")
...
def send_in_window(
    ...
    time.sleep(random.uniform(0.1, 0.3))
    logger.debug(f"Deity-specific generation - message: {message}")
    human_type(message)
